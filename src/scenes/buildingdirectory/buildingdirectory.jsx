...
const BuildingDirectory = (props) => {
	...
	function ItemTemplate(data) {
		const textColor = theme.palette.mode === "dark" ? "#fff" : "rgba(0, 0, 0, 0.87)";
		...
		return (
			<div style={{ 
				...
				onClick={
					() => navigate('/floorInfo', {state:{building_name: data.building_name}})
				}
			>
	...
	const getBuildingData = async () => {
		const response = await axios.get('http://localhost:3002/v1/api/building-details');
			...
			<Box m="20px">
				...
				<Box display="flex" justifyContent="space-between" alignItems="center">
					...
					<IconButton
						...
						sx={{ bgcolor: colors.greenAccent[500], color: 'white', marginLeft: '10px'  }}
					>
						<AddIcon />
					</IconButton>
					<AddBuildingModal
